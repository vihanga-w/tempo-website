    ...
    useEffect(() => {
        ...
        const updateState = (data: UpdateEvent) => {
            ...
            if (data.data.state?.replayCount && data.data.state?.replayCount > 0) {
                ...
                setUserListenershipFact({
                    sid: data.data.state?.songId ?? "",
                    text: (new Date().getTime() - data.data.state.playSessionStart >= (60e3 * 5) ? `Listened for ${formatTimeToMinAndHour(new Date().getTime() - data.data.state.playSessionStart)} today` : "Started listening recently"),
                });
